...
const FilterButtons = ({ filterData, flag }) => {
  ...
  return (
    <div className="border border-amber-600 rounded-md px-4 py-1 relative cursor-pointer hover:border-gray-400 ">
      <p
      ...
      {filterData && filterMenu && (
        <motion.div
          ...
          exit={{ opacity: 0, y: 50 }}
          className="w-48 z-50 backdrop-blur-sm max-h-44 overflow-y-scroll scrollbar-thin scrollbar-track-gray-200 scrollbar-thumb-gray-400 py-2 flex flex-col rounded-md shadow-md absolute top-8 left-0 bg-amber-600"
        >
          {filterData?.map((data) => (
            <div
              key={data.name}
